...
class Line(object):
    ...
    def nli_generation(self, signal_power, Rs, df, Bn=12.5e9):
        ...
        eta = 16 / (27 * pi) * self.gamma ** 2 / (4 * self.alpha * self.beta * Rs ** 3) * np.log(
    ...
    def stream(self, connections, best='latency', transceiver='shannon'):
        ...
        for connection in connections:
            ...
            if path:
                ...
                channel = [i for i in range(len(path_occupancy)) if path_occupancy[i] == 'free'][
                    0]  # take the first available channel
                path = path.replace('->', '')
                ...
                if out_lightpath.bitrate == 0.0:
                    [self.update_route_space(lp.path, lp.channel, 'free') for lp in
                     connection.lightpaths]  # set everything we tried to occupy in the path if we fail to get the whole requested rate
...
def main():
    NMC = 2  # number of monti carlo simulation 10 20 30 50 100 500
    node_pairs_realizations = []
    stream_conn_list = []
    lines_state_list = []
    for i in range(NMC):
        print('Monte - Carlo Realization #{:d}'.format(i + 1))
        network = Network('nodes.json', nch=20, upgrade_line='AC')  # number of channels
